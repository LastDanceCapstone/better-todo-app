...
async function callOpenAI(model: string, systemPrompt: string, userContent: string): Promise<string> {
  ...
  try {
    ...
    console.error('[ai.parse-task] OpenAI request failed', err?.message || err);
    throw new ProviderRequestError('Failed to reach AI provider');
  }

  const body = (await response.json()) as OpenAIChatCompletionResponse & { error?: { message?: string; code?: string } };
  if (!response.ok) {
    const msg = body?.error?.message || `status ${response.status}`;
    console.error('[ai.parse-task] OpenAI error', response.status, msg);
    if (response.status === 401) {
      throw new ProviderRequestError('AI provider API key is invalid or missing');
    }
    if (response.status === 429) {
      throw new ProviderRequestError('AI provider rate limit reached. Try again in a moment.');
    }
    throw new ProviderRequestError(`AI provider request failed: ${msg}`);
  }

  const rawContent = body.choices?.[0]?.message?.content;

  if (!rawContent || typeof rawContent !== 'string') {
    console.error('[ai.parse-task] OpenAI returned empty or invalid content');
